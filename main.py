...
def assign_new_bbs(cars, bbs, thresh=10):
    ...
    for car in cars:
        ...
        if min_dist>=0 and min_dist<=thresh: # assign nearest_c to car
            ...
            car.w = bbs[nearest_c_idx][1][0] - bbs[nearest_c_idx][0][0]
            car.h = bbs[nearest_c_idx][1][1] - bbs[nearest_c_idx][0][1]
            car.age = 0
            car.M += 1
            #print("Assigned: ({},{})".format(car.centroid[0],car.centroid[1]))
            assigned[nearest_c_idx] = True
    ...
    for bb in [bb for bb,ass in zip(bbs, assigned) if not ass]:
        ...
        car.w = bb[1][0] - bbs[0][0]
        car.h = bb[1][1] - bbs[0][1]
        car.id = len(cars)
        #print("Created: ({},{})".format(c[0],c[1]))
        cars.append(car)
...
def draw_cars(img, cars, thresh=3):
    out_img = np.copy(img)
...
def process_image(img):
    ...
    print("---")
    bboxes = find_car_bboxes(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins)
    ...
    #    centroids.append(((bb[0][0]+bb[1][0])//2, (bb[0][1]+bb[1][1])//2))
    assign_new_bbs(cars, bboxes_nms, thresh=20)
    out_img = draw_cars(img, cars)
